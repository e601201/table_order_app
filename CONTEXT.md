...
A `Staff` role for system administration rather than `Order` operation. Unlike `Kitchen` and `Cashier`, it advances neither order axis. Conceptually an `Admin` may manage `Staff` accounts, manage the `Menu`, manage the `決済方法` master, and reach every staff surface. Today an `Admin` can manage `Staff` accounts, log in, and manage the `Menu` (create / edit / delete `MenuItem`s, including images) via `/admin/menu_items` (ADR-0004) — including each `MenuItem`'s availability (`Stock`, `Suspended`, and the derived `Sold out`; `Restock`), which **only** an `Admin` may change — and read-only-monitor the day's `Order`s via `/admin/orders` — viewing both state axes and totals without advancing either (ADR-0005). The order screen is a single-restaurant daily oversight view: there is no Store/Shop concept and no cancellation state (ADR-0005). An `Admin`'s home is a statistics dashboard at `/admin/dashboard` showing the day's KPIs (sales / order count / 平均注文単価, each with a day-over-day change), a 7-day sales trend, the day's top-5 popular `MenuItem`s by sold quantity, and a read-only preview of recent `Order`s that links to `/admin/orders`; navigation is delegated to the admin sidebar and the staff-surface switcher, not link cards. An admin-only switcher in each staff surface's header lets it move between `Kitchen`, `Cashier`, and the admin console (ADR-0005 update).
...
The `Takeout` `Customer`'s authenticated, cross-visit list of their own past and in-flight `Order`s, served at `/order/history` and bound to the `LineAccount` (ADR-0008). Persists across visits, requires LINE login, and shows the payment terminal — a `Closed` order appears with the single 「キャンセル」 badge (see `Closed`). Distinct from `注文状況`: identity-bound vs `session`-bound, cross-visit vs in-visit, `Takeout` vs `In-store`.
_Avoid_: 注文状況 / Order status (the `In-store` in-visit surface — the opposite member of the pair), History alone
...
- **決済** — reserved for **compound technical terms** ("クレジット決済", "決済方法" — see `決済方法`). Never use bare "決済" for the act itself.

_Avoid_: Bare "決済" for the act, "会計" for the role (the role is "レジ" — see Roles)

**決済方法** (canonical Japanese: **決済方法**, English/code: **PaymentMethod**):
`会計`（Settlement）でお金を受け取る手段の名前（現金、クレジットカード、PayPay …）。`Admin` が管理するマスタで、`Cashier` が `Settlement` の瞬間に1つ選ぶ。振る舞いを持たない純粋な記録ラベルであり、`Order` は `Settlement` 時に方法**名**をスナップショットする — マスタを後から改名・削除しても、会計済みの歴史は1文字も変わらない（`Checkout` で `Line` が `MenuItem` をスナップショットするのと同じ哲学）。ライフサイクルは `MenuItem` と同型: 有効/無効トグル（端末故障などの運用上の一時停止）と、ADR-0003 流の物理削除（恒久撤去）の2段構え。不変条件はただ1つ「**有効な決済方法が最低1つ**」 — レジが `Paid` を書けない状態は存在できない。現金の特別扱いはない（キャッシュレス専門は正当な構成）。無効化・削除済みの方法での `Settlement` は拒否される — 黙って別の方法に読み替えることはない（会計記録が嘘をつくため）。
_Avoid_: 支払い方法 as the canonical / staff-surface name（支払い は客向けコピー専用 — 客の目に入るコピーでのみ「お支払い方法」を許容）, bare 決済, 支払い手段 / 決済手段（canonical は 決済方法）, payment type
